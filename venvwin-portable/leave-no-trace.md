# venvWin Portable Leave-No-Trace Policy

## Product rule

venvWin Portable is a lightweight portable Linux environment for making old PCs useful again.
...
Boot from USB.
Run from USB/RAM.
Store venvWin state on venvWin-owned storage.
Do not consume or modify the host PC hard drive unless the user explicitly chooses it.
```

## Target feel

venvWin should feel closer to the useful old Puppy Linux era than a bloated modern desktop distro.
...
- clear logs and proof inside venvWin storage/session
...
- keep capsules, logs, and proof files on venvWin-owned storage when possible

LNT does not mean:

- hiding activity
- disabling logs
- removing evidence from venvWin-owned storage
- bypassing auditability
- behaving like hacker trash

## Default writable target

Default writable state should go to venvWin-owned storage:

```text
VENVWINDATA/venvWin-Capsules
```

or another detected venvWin-owned persistent storage path.
...
- install venvWin/venvWin onto host

## Traceability policy

venvWin should create and preserve evidence of use in venvWin-owned locations.
...
These artifacts are for clarity, debugging, support, and honest traceability. They should live in the venvWin session or venvWin-owned storage, not sprayed across the host PC.

## Desktop/browser residue rule

venvWin should not leave annoying residue on the host.
...
venvWin desktop proof files are acceptable because they exist inside the venvWin live desktop/session, not on the host OS desktop.
...
```text
preferred: /media/<user>/VENVWINDATA/venvWin-Capsules
fallback: /run/live/persistence/venvWin-Capsules
last resort: ~/venvWin-Capsules with disposable warning
```
...
If venvWin cannot find writable venvWin-owned storage:

```text
Warning: no venvWin-owned persistent storage found. This session may be disposable. Installed apps may vanish after reboot. Use this for testing, not real work.
```
...
- `/media/<user>/<host-drive-label>` unless label is `VENVWINDATA` or user confirms
- `/mnt/*` unless mounted by venvWin persistence logic
...
1. Use detected venvWin USB storage
2. Create/use venvWin-Capsules on VENVWINDATA
...
venvWin Portable should feel safe to boot on an old or borrowed machine without making a mess:
...
No mystery writes.
Proof and logs remain in venvWin-owned storage/session.
```
