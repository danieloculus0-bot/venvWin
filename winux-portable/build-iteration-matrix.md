# WinUx Portable Build Iteration Matrix

## Purpose

WinUx Portable needs a repeatable comparison loop so each build gets judged against the right target instead of vibes, caffeine, and goblin prophecy.
...
|---|---:|---:|---|---|
| core | smallest useful WinUx shell | lowest | rescue/dev/minimal base | no Windows runner if Wine omitted |
...
If the build gets smaller but loses the reason WinUx exists, that is not optimization. That is self-sabotage with a file-size fetish.
