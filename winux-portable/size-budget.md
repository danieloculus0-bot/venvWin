...
WinUx Portable uses that as the design benchmark, but not as a literal byte-for-byte target.

Puppy could stay tiny because it did not ship the same payload WinUx needs:
...
Do not remove the reason WinUx exists just to win a file-size contest.
